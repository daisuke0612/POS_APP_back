...
class Transaction(Base):
    __tablename__ = 'transactions_hase'
...
class TransactionDetail(Base):
    __tablename__ = 'transaction_details_hase'

    DTL_ID = Column(Integer, primary_key=True, autoincrement=True)  # Detail ID
    TRD_ID = Column(Integer, ForeignKey('transactions_hase.TRD_ID', ondelete="CASCADE"), nullable=False)  # Transaction ID (Foreign Key)
    PRD_ID = Column(Integer, ForeignKey('m_product_hase.PRD_ID', ondelete="CASCADE"), nullable=False)  # Product ID (Foreign Key)
